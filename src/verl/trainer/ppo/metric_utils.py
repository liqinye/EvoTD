...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    non_aborted_format_score = batch.non_tensor_batch.get("format", None)[non_aborted_mask]

    format_score_mean = torch.mean(torch.tensor(non_aborted_format_score)).detach().item()
    ...
    # Raw reward (before overlong penalty) if available
    reward_raw = batch.non_tensor_batch.get("reward_raw", None)
    reward_raw_metrics = {}
    if reward_raw is not None and len(reward_raw) == len(sequence_score):
        if not isinstance(reward_raw, torch.Tensor):
            reward_raw = torch.tensor(reward_raw)
        reward_raw = reward_raw.float()
        reward_raw_non_aborted = reward_raw[non_aborted_mask]
        if reward_raw_non_aborted.numel() > 0:
            reward_raw_metrics = {
                "critic/reward_raw/mean": torch.mean(reward_raw_non_aborted).detach().item(),
                "critic/reward_raw/max": torch.max(reward_raw_non_aborted).detach().item(),
                "critic/reward_raw/min": torch.min(reward_raw_non_aborted).detach().item(),
            }
    ...
    metrics = {
        ...
        # format
        "critic/format/mean": format_score_mean,
        # reward
    ...
    format_scores = batch.non_tensor_batch.get("format")
    if task_types is not None:
        if isinstance(task_types, torch.Tensor):
            task_types = task_types.cpu().tolist()
        elif isinstance(task_types, np.ndarray):
            task_types = task_types.tolist()
        else:
            task_types = list(task_types)
        if format_scores is not None:
            if isinstance(format_scores, torch.Tensor):
                format_scores = format_scores.detach().cpu().tolist()
            elif isinstance(format_scores, np.ndarray):
                format_scores = format_scores.tolist()
            else:
                format_scores = list(format_scores)
        format_scores_valid = format_scores is not None and len(format_scores) == len(sequence_score)
        if len(task_types) == len(sequence_score):
            per_task_stats: dict[str, dict[str, float]] = {}
            for idx, task in enumerate(task_types):
                if not bool(non_aborted_mask[idx]):
                    continue
                ...
                val = sequence_score[idx].detach().item()
                stats["sum"] += val
                stats["count"] += 1
                stats["max"] = max(stats["max"], val)
                stats["min"] = min(stats["min"], val)
                if reward_raw is not None and idx < len(reward_raw):
                    raw_val = reward_raw[idx].detach().item()
                    stats["raw_sum"] += raw_val
                    stats["raw_count"] += 1
                    stats["raw_max"] = max(stats["raw_max"], raw_val)
                    stats["raw_min"] = min(stats["raw_min"], raw_val)
                if format_scores_valid:
                    format_val = format_scores[idx]
                    if format_val is not None:
                        stats["format_sum"] += float(format_val)
                        stats["format_count"] += 1
            for task_name, stats in per_task_stats.items():
                if stats["count"] == 0:
                    continue
                metrics[f"critic/score/{task_name}/mean"] = stats["sum"] / stats["count"]
                metrics[f"critic/score/{task_name}/max"] = stats["max"]
                metrics[f"critic/score/{task_name}/min"] = stats["min"]
                metrics[f"critic/score/{task_name}/count"] = stats["count"]
                if stats["raw_count"] > 0:
                    metrics[f"critic/reward_raw/{task_name}/mean"] = stats["raw_sum"] / stats["raw_count"]
                    metrics[f"critic/reward_raw/{task_name}/max"] = stats["raw_max"]
                    metrics[f"critic/reward_raw/{task_name}/min"] = stats["raw_min"]
                    metrics[f"critic/reward_raw/{task_name}/count"] = stats["raw_count"]
                if stats["format_count"] > 0:
                    metrics[f"critic/format/{task_name}/mean"] = stats["format_sum"] / stats["format_count"]
