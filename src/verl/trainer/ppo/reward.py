...
def get_custom_reward_fn(config: DictConfig) -> Optional[RawRewardFn]:
    ...
    # ======= ADD ======    
    # Auto-pass BASE flag to custom reward_fn if it expects it and not provided explicitly.
    if "BASE" in inspect.signature(raw_fn).parameters and "BASE" not in reward_kwargs:
        base_flag = False
        try:
            base_flag = bool(config.data.get("apply_chat_template_kwargs", {}).get("BASE", False))
        except Exception:
            pass
        reward_kwargs["BASE"] = base_flag
    # ======= ADD ======    
