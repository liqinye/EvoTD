...
def parse_args():
    ...
    base_op_parser.add_argument(
        "--is-value-model",
        action="store_true",
        help="Whether the model is a value model",
    )
    base_op_parser.add_argument(
...
class ModelMergerConfig:
    ...
    Args:
        ...
        tie_word_embedding (bool): Whether to tie word embedding weights. Defaults to False.
        trust_remote_code (bool): Whether to trust remote code. Defaults to False.
        is_value_model (bool): Whether the model is a value model. Defaults to False.
...
class BaseModelMerger(ABC):
    ...
    distributed training backends (FSDP) into standard HuggingFace format that
