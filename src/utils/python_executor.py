...
from utils.equality import deep_equal, to_bool
...
from utils.checks import contains_banned_imports
from utils.parsers import parse_error
...
class PythonExecutor:
    ...
    def eval_input_prediction(self, code: str, gold_output: str, agent_input: str, imports: List[str] = []) -> float:
        ...
        for retry in range(max_retries):
            try:
                ...
                return self._boolean_score(correct, status)
    ...
    def eval_output_prediction(self, code: str, gold_output: str, agent_output: str, imports: List[str] = []) -> float:
        try: # fast check if we dont need to run the code
            if self._safe_literal_equal(gold_output, agent_output):
    ...
    def eval_k_input_prediction(self, code: str, gold_output: str, k_agent_inputs: List[str], imports: List[str] = []) -> List[float]:
        ...
        if 'error' in status.lower():
            ...
            # print(f"Error gold_output: {gold_output}")
            # print(f"Error k_agent_inputs: {k_agent_inputs}")
            # print(f"Error status: {status}")
            # print(f"Error acc_list: {acc_list}")
            pass
        output_acc = self._parse_boolean_list(acc_list, len(valid_k_agent_inputs)) + invalid_lists
    ...
    def eval_k_output_prediction(self, code: str, gold_output: str, k_agent_outputs: List[str], imports: List[str] = []) -> List[float]:
        ...
        if 'error' in status.lower() or 'exception' in status.lower():
            # print(f"Error code: {code}")
            # print(f"Error gold_output: {gold_output}")
            # print(f"Error k_agent_outputs: {k_agent_outputs}")
            # print(f"Error status: {status}")
            # print(f"Error acc_list: {acc_list}")
            pass
        output_acc = self._parse_boolean_list(acc_list, len(valid_k_agent_outputs)) + invalid_lists
        # output_acc = eval(acc_list) + invalid_lists
    ...
    def check_all(
        ...
        if check_error:
            ...
            if isinstance(output, str) and output.strip() == "":
                return False, None
            return not ('error' in status.lower() or 'exception' in status.lower()), output
    ...
    def execute(
        ...
        try:
            if auto_mode:
                # Always capture stdout to avoid leaking user prints to worker logs
    ...
    @staticmethod
    def truncate(s, max_length=1000000):
        half = max_length // 2
        if len(s) > max_length:
            s = s[:half] + "..." + s[-half:]
        return s
    ...
    # ================================ Helper Functions ================================
    def _boolean_score(self, correct: str, status: str) -> float:
        if not status or 'error' in status.lower() or 'exception' in status.lower():
            return 0.0
        verdict = to_bool(correct)
        return 1.0 if verdict else 0.0

    def _safe_literal_equal(self, gold_output: str, agent_output: str) -> bool:
        try:
            gold = ast.literal_eval(gold_output)
            agent = ast.literal_eval(agent_output)
        except Exception:
            return False
        return deep_equal(gold, agent)

    def _parse_boolean_list(self, acc_list: str, expected_length: int) -> List[float]:
        try:
            parsed = ast.literal_eval(acc_list)
        except Exception:
            return [0.0] * expected_length
        if not isinstance(parsed, list):
            parsed = [parsed]
        bool_values = [1.0 if to_bool(value) else 0.0 for value in parsed]
        if len(bool_values) < expected_length:
            bool_values.extend([0.0] * (expected_length - len(bool_values)))
        elif len(bool_values) > expected_length:
            bool_values = bool_values[:expected_length]
        return bool_values
